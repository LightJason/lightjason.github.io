...
It is possible to create _hierarchical structured_ belief bases by using the slash `/` as a separator, e.g. to distinguish between internal and external knowledge (`internal/preferredspeed(120)`, `external/intendedspeed(100)`). This feature **requires** to add *Storage containers* to the beliefbase which reflect the hierarchical structure of beliefs. If these containers do not exists, the default behaviour is to ignore the belief-addition to the beliefbase. See the _[&#8594; Tutorial: Efficient Beliefbase](/tutorials/efficient-beliefbase)_ for details on *Storage containers*.
