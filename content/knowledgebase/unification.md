...
> ```Minute``` the number $0$ and the variable ```Period``` the atom `pm`.
...
In AgentSpeak(L++) unification can be done by using the right shift operator `>>` in combination with a literal to be unified. Values of interest are then replaced by [variables](../variables).
